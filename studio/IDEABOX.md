...
- `[new]` (2026-06-25) **Hunting — and maybe hunger/thirst/death?** Operator nudge, dropped raw. Dinos could *hunt* — predator/prey, a carnivore stalks and a herbivore flees, the food web the cast currently doesn't have (everything eats from the keeper's hatch or the plots). And the bigger, less-sure question behind it: should dinos have **hunger and thirst** drives (a need that builds, pushes them toward food/water/the pond, goes bad if unmet) — and does unmet need eventually mean **death** (a dino can actually die, the bowl isn't deathless)? The `...?` is the operator's own uncertainty — these may be too dark / too sim-heavy for a cozy fishbowl, or exactly the stakes that make it alive. Lore-smith/Structure-smith: weigh hard against the CHARTER's cozy-companion vibe (Stardew/Pokemon, "quiet desktop companion" per idle mode 060) and the no-half-features rule — hunger as a *gentle pull* toward the pond/plots/hatch (feeds 359's pond, the grove economy) is very different from a survival-sim death spiral. Split it: a need-drive *spine* (hunger/thirst as a trait-shaped pull, no death) is structural and shippable small; predator/prey hunting is an emergent beat; death is a charter-level call the operator may need to rule on directly (it touches breeding/eggs, the plaque lineage, save permanence). Shape, split, defer, or route to the operator as you see fit.
