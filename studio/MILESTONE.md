...
- [ ] The suite can wait for time — the runner stops losing specs at both extremes of load, at the timing seam a fast serial boot reaches sooner than a loaded one (BACKLOG-515, -430) — *skipped cycle 147 for a stated reason (its bar answer is nothing a player sees) and now the only open structure arc. It has cost four different specs a full-run red. The cycle-147 verdict rules it does not get a fifth skip on the same argument without somebody writing down what the park loses by carrying it.*
