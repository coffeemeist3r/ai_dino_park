...
- [ ] BACKLOG-157 [emergent] More keeper abilities — beyond the affinity-fit of 155, the distinct per-observer powers the operator floated: one scans/reads dino stats, one sees the bond graph, one nudges the weather/sky event. One ability per cycle, each a real read on which observer you chose. Builds on 155 / 144 / 021. *(progress: **2/3** — LUMEN-3's **Field Scan** shipped cycle 38 (B reads a dino's axes/mood/favorite/role; the others refuse in character). AETHER-1's **Read the Room** shipped cycle 164 (R reads the floor of the ground you are standing on: who is standing near whom and whether that pair is at ease or edgy, plus whoever nobody is near — `keeper/room.ts`, reusing `stargazingPairs` at a wider radius so the same-zone guard is shared rather than copied; VANTA-9/LUMEN-3/Kes refuse in character). **Note for the third fire: the remaining candidate is VANTA-9's sky-nudge, and it is not free.** Cycle 164 declined it on a CHARTER v7 argument worth not re-deriving: the flare's payoff is gated on a clear night, a fresh save opens at `FOUNDING_HOUR` = 8, and at `ACTIVE_SCALE` = 60 dusk is about **twelve real minutes** out — past the ten-minute reachability bar. So the third ability either ships with something a player can reach before dusk (a refusal is not a payoff), or it waits for a cycle that also hands the flare a reachable night. Kes has no ability yet either and is the other open seat. Whichever is taken, the touch surface cannot carry it: BACKLOG-552's More sheet is at its geometric ceiling and `R` is the second keeper verb in three cycles to ship keyboard-only.)*
