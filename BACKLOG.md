...
- [ ] BACKLOG-505 [core] The frontier 500 closed — putting a resident on every ground made 474's unsettled-ground read, its lens glyph and the frontier migration tier dormant on a fresh save (full text in the cycle-140 block below).
...
- [x] BACKLOG-503 [core] The branch with nothing to choose — the Ridge is the only ground reached by a *choice* rather than by walking the line, and nothing lives there or grows there that the line cannot supply (full text in the cycle-140 block below).
...
- [x] BACKLOG-508 [art] The black glass and the thing built from it — the Ridge's obsidian shard + the beacon raised from it, both shipping on the glyph fallback (full text in the cycle-142 block below).
