...
>
> **Amended cycle 145 — the rule now has a deadline.** BACKLOG-501's reachability register carries an
> entry that fails on any `PROP_RIGS` key the shipping world cannot place, and its first walk went dark
...
- [ ] BACKLOG-543 [art] The sulk, in pixels — `fidget.ts` has carried `MOOD_GLYPH = { sulk: '😒', cold: '🥶' }` since BACKLOG-310, and the contested-drop loser gets a bare 😤 flashed over its head from `WorldScene`. **CORRECTED cycle 156-art — the seed's host claim was false and is the reason this was not drawn.** Neither ὡ2 nor ὢ4 is a *mark*. The sulk is a `mark.setText()` on the **activity glyph** (`WorldScene.ts:5758-5762`, `moodFidget`), and the standoff ὢ4 is a one-frame `flashFeed`. Neither goes through `makeHourMark`, so neither has a rig lookup: a `sulk` key added to `PROP_RIGS` tonight would land in `unplacedRigs()` and redden `cycle-145-reachability.test.ts`. **The host this needs is a `refreshSulkMarks`** — the shape `refreshMendMarks` (530) and `refreshGlanceMarks` (119) already have — hanging the mark over the sulker for the length of BACKLOG-123's two-minute window. That is main-chain scene glue and is the Structure-smith's or Designer's to build, exactly as cycle 154's BACKLOG-530 rider built `mend`'s host the morning `mend` was drawn. **STILL BLOCKED — cycle 157-art, and the false claim this time was the Validator's own, made an hour earlier in the same cycle.** The cycle-157 verdict declared this unblocked on the grounds that BACKLOG-544's `shoulder` funk gave the sulk a durable state with a live visual host. **The state is real; the host is not.** The 😒 is still a `setText` on `activityMarks`, which is declared `Phaser.GameObjects.Text[]` — a typed array of Text objects. `makeHourMark` is what swaps a Text for an `Image` when `hasPropArt(key)` is true, and the activity mark does not go through it, so a rig has nowhere to be blitted. Worse, `worldPlacedProps()` in `reachability.ts` is a hand-maintained list: a `sulk` key would land in `unplacedRigs()` and redden `cycle-145-reachability.test.ts`, and adding `sulk` to `worldPlacedProps` to silence it would be a **false entry in the register that exists to catch false entries**. What this needs is unchanged from the cycle-156 correction and is now more precisely named: a mood mark built by `makeHourMark` (the `mend`/`glance`/`vigil` shape), refreshed over a dino for the length of its funk, reading the 544 funk record — which now exists and is the half that was genuinely missing. That is main-chain scene glue, not the Artist's. Draw it the cycle the mark exists. The six sibling marks (`mend`, `doze`, `missed`, `rouse`, `vigil`, `wave`) are all authored pixel rigs, so the funk is still the loudest un-drawn thing in the family — it just needs somewhere to hang. Draw it in the family's palette as a **turned-away head** rather than a frown — the whole family reads at 12px and a mouth does not survive that, but a head rotated off the player does. One rig serving both keys (`sulk`, `shoulder`), or one rig plus a flip, at the sibling weights and with its lit-cell count claimed against **every** sibling per the cycle-155 correction. Standalone via `bakePropArt`; the host predates the seed by 125 cycles. Builds on 310 / 120 / 062 / 540.
...
- [ ] BACKLOG-556 [art] The loner's wilt, in pixels — `MOPE_GLYPH = '🥀'` (`world/loner.ts:23`) is hung over any dino the bond graph says is friendless (`refreshMopeMarks`, `WorldScene.ts:4395`), and it is now the loudest un-drawn mark in the family: every founding dino on a fresh save is friendless, so on the first morning of a brand-new park **five** raw system emoji are floating over the cast at once. Every sibling — `doze`, `rouse`, `vigil`, `missed`, `mend`, `glance`, and as of cycle 162 both need tells — is an authored pixel rig. This one is not, for exactly one reason: line 3802 builds it as `this.add.text(0, 0, MOPE_GLYPH, …)` instead of `this.makeHourMark(...)`, so there is no rig lookup on that path. **Seeded with its host named and its host scheduled.** Unlike the sulk (543), which needs a whole new refresh over a shared `activityMarks` array, and unlike the plaque (539), which needs a per-line object that does not exist, this host is the smallest one the art queue has ever carried: `mopeMarks` is already a dedicated array with a dedicated refresh and — crucially — a **single** key, so it needs no two-key swap at all, which is less than BACKLOG-551 had to do for the need tells. Four lines: widen `mopeMarks` to `(Text | Image)[]`, build it with `makeHourMark(MOPE_ART_KEY, MOPE_GLYPH)`, export `MOPE_ART_KEY = 'mope'` from `loner.ts`, and add it to `worldPlacedProps()`. **That rider is this cycle's Coder's**, on the cycle-154 BACKLOG-530 precedent where the main chain built `mend`'s host the morning `mend` was drawn — so the Artist should find the host live rather than the queue holding a third blocked mark. Draw it at the family's sibling weights and lit-cell discipline: a **drooping stem**, one head bowed past its own stalk — the family reads at 12px, where petals do not survive but a bent line does, and the bend is the whole idea. Standalone via `bakePropArt`. Builds on 135 / 551 / 550 / 520.
...
- [ ] BACKLOG-533 [infra] The fixture nobody is required to name — BACKLOG-495 gave the suite `foundingState(page, name)` with four declared fixtures, each carrying a postcondition, and moved thirty-nine spec files onto it without editing one of them. What it deliberately did **not** ship is a rule: a spec that says nothing still means `'as-shipped'` by silence rather than by declaration, which is three-quarters of the original complaint answered and one quarter left standing. The scoping call was right at the time — a rule that everything use a seam is noise until the seam has been used — but the seam has now been used, by every spec in the suite, and the next founding-constant move is the experiment that says whether naming happens without being asked. Options, in ascending cost: a lint over `tests/e2e/*.spec.ts` requiring one `foundingState` call per file; a `boot()` that takes the fixture name as a required argument (loud, ~250 call sites, and the compiler does the work); or nothing, if the next move comes back clean. Decide from evidence: count how many specs the *next* founding-constant move reddens, and compare against cycle 136's sixteen and cycle 151's three. Builds on 495. **ENTRY CONDITION REWRITTEN — cycle 158, the night its deadline came due.** Passed over three times (155, 156, 157), every time for the same honest reason, and the cycle-157 Validator named it: the evidence is not being generated. No cycle has scheduled a founding-constant move, so the trigger was a wait on an event nobody had committed to causing — which is not a condition, it is a way of never taking an item. The evidence clause is demoted to a **tie-break on which of the three options to take**; the gate becomes a **date**. **Taken at cycle 165, or at the first cycle whose structure item edits a founding constant, whichever comes first.** If a founding-constant move has happened by then, its red count picks the option. If none has, **the lint ships** — it is the cheapest of the three and it generates the evidence the other two were waiting on.


## Cycle 163 structure additions — the watcher is a save field (2026-09-17)

- [~] BACKLOG-555 [core] The watcher's record, not just its id — `saveGame.ts` persists exactly one thing about the observer: `keeperId?: string` (line 81, additive over v1 at line 322). That was right for BACKLOG-155, which only needed to remember the pick. It is the reason **two** queued keeper items have nowhere to land. BACKLOG-156 asks for an LLM-authored keeper persona under the CHARTER's generate-once/cache/persist rule — there is no field to cache it in, so today it would be re-authored on every boot or not persisted at all, which is the failure mode "Living minds" exists to forbid. BACKLOG-162 asks the bowl to notice when the watcher *changes* — there is no previous id, no switch count and no timestamp, so "the watcher changed" is not derivable from the save at all; a load looks identical whether you have worn one chassis for a month or four in an hour. Ship the record: a `keeper` object beside `keeperId` carrying, at minimum, when the current observer was chosen (in the game's own day/step terms, the units the plaque and the streak already use), how many times the watcher has changed, the previous id, and a slot the 156 persona cache can occupy without another save migration. **Strictly additive** — an old save with a bare `keeperId` loads to a record seeded from it, and the CHARTER's additive-save rule is the point of the item, not a constraint on it. Keep it pure and Node-testable in `world/saveGame.ts` + `keeper/`, no Phaser. The reachability half is the part to get right rather than defer: a record nothing reads is groundwork, and CHARTER v7 calls groundwork a REWORK — ship it with **one** line that reads it, in the plaque or the book, so a fresh save shows the keeper's tenure the way it already shows the park's. Builds on 155 / 156 / 162 / 212.
