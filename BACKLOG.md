...
- [ ] BACKLOG-519 [infra] The day nobody exports — the reachability register keeps its own copy of the length of a day, in the one file whose header is a lecture about second copies (full text in the cycle-145 block below).
...
- [ ] BACKLOG-520 [art] Asleep in the wrong hours — the behaviour glyph for the day/night split (109). The park already proves this shape three times over: `tic_pace`, `tic_circle` and `tic_fuss` are 16px behaviour marks that sit over a dino and say what it is doing without a word of text. Give the hours the same. **Two states, one family.** A `doze` for a dino down outside its own hours — the heavy one, eyes shut, a slow exhale mark, warm and low-contrast so it reads as *out of it* from across the field. And a `rouse` for a dino up while its neighbours are down — the light one, head raised, one open eye catching whatever light there is, cool and high-contrast, because the whole point of a night-owl is that it is the only thing moving. They must be tellable apart at 32px with no lens, which is the bar `pile_1..3` set. **Host: BACKLOG-109, shipping in this same cycle 146** — per the cycle-145 amendment, this is seeded with its host named and dated rather than left to a later cycle to find one. If 109 does not land, this item waits with 518 rather than shipping into a red register. Builds on 109 / 405 / 496.
...
## Cycle 145 — filed by the Validator (2026-08-30)

> One finding from the build. 501 shipped a file whose header is a lecture about a claim written down twice
> going stale in one of the two places, and it had to write a number down twice to do it.

- [ ] BACKLOG-519 [infra] The day nobody exports — `reachability.ts` checks BACKLOG-493's claim (a whole in-game day fits inside a session) by dividing `24 * 60` by `ACTIVE_SCALE`, because `MINUTES_PER_DAY` is a **private** const in `clock.ts`. So the one file in the park whose own header warns that a claim written down twice goes stale in one of the two places contains a second copy of the length of a day. It is the mildest possible instance — a day is a definition, not a tuning knob, and nobody is going to redefine it to 23 hours — which is exactly why it is worth fixing while it is cheap: the register's authority rests on every entry going through the production function that owns its fact, and this is the only entry that does not. The fix is a one-line `export` in `clock.ts` and an import. QA logged it rather than failing the criterion, correctly. While in there, consider whether `SESSION_MINUTES` belongs beside `ACTIVE_SCALE` too — it is the number that says what "watchable" means, and it currently lives in the register rather than beside the clock it judges. Builds on 501 / 493.
