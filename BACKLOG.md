...
- [ ] BACKLOG-550 [art] Hunger and thirst, in pixels — the need-drive tells (371) are the last two marks in the park still rendering as raw system emoji. `refreshNeedMarks` (`WorldScene.ts:4241`) hangs `NEED_GLYPH.hunger` 🍖 or `NEED_GLYPH.thirst` 💧 over a dino in want, above the cold/mope slot, and it is the *most-seen* un-drawn thing in the game: every dino carries a need, needs build on every tick, and the mark is up long before anything else in the mark family fires. **Seeded knowingly blocked, cycle 160, with the host named.** The mark is built inline as `this.add.text(0, 0, '', …)` at `WorldScene.ts:3651`, so there is no rig lookup and a `need_hunger` key added to `PROP_RIGS` tonight would land in `unplacedRigs()` and redden `cycle-145-reachability.test.ts` — the same trap 543 and 539 sit in. **But this one's host is the cheap one.** Unlike the sulk's shared `activityMarks`, `needMarks` is already a dedicated array with a dedicated refresh; routing it through `makeHourMark` and giving it a two-key swap is *literally* the shape `refreshMissedMarks` already runs for `missed` / `missed_aloof`, and is about ten lines of scene glue plus two `worldPlacedProps()` entries. That is a Structure-track rider, not the Artist's to write (routine 7: "don't touch game logic beyond the thin bake.ts wiring"). Draw it the cycle the mark exists. Two rigs at the family's sibling weights and lit-cell discipline: a **belly** read for hunger (the family reads at 12px, so a haunch of meat is the wrong idea — a hollow curve is not) and a **droplet with a tongue** for thirst. Standalone via `bakePropArt`. Builds on 371 / 436 / 520 / 531.
...
- [~] BACKLOG-538 [infra] The victim moves again — the parallel-load boot flake is back, and the record is what makes it worth an item rather than a footnote. Cycle 148 shipped BACKLOG-515/430 and recorded **649/649 in both directions, this project's first all-green run**; its verdict said the mechanism had been *exhausted*. Cycle 152 ran **670/670**. Cycle 153 is **675 specs** and dropped exactly one on each of two consecutive full runs, with a **different victim each time** — `cycle-082-comfort-food.spec.ts:24` (`waitForFunction` on `__ready`) then `cycle-121-yearning.spec.ts:46` (`canvas` visibility) — both failing inside `helpers.boot`, before either spec's own subject exists, and both green when re-run isolated (3/3 and 7/7). That is the one-victim-per-run signature `helpers.ts` documents in its own comment. **The first deliverable is a reproduction, not a fix.** 515 was re-diagnosed across four cycles by four routines that each had ten minutes for it, and it was solved only when somebody could reproduce it on demand and watch the victim move; the honest hypothesis to test first is that 515's repair bought *headroom* rather than a floor and the suite has grown back into the seam (the cold-boot budget against `BOOT_TIMEOUT`, not the input race 515 closed). Five specs were added the night this was filed; that is the only nameable variable and it is a weak one — the victims were not them. Filed by the cycle-153 Validator on QA's declared trend, deliberately as an observation with two data points rather than a diagnosis. Builds on 515 / 430. **UNBLOCKED at cycle 159 by Validator ruling:** CHARTER v7's reachability bar has a reading for `[infra]` — a substituted question, *what can the next cycle do that it could not do before, and what is the evidence produced in this cycle that it works* — with one hard condition: the deliverable must be **run and demonstrated inside the cycle that ships it**, not described. A harness that reproduces the flake on demand, with the verdict carrying the run and the victim it caught, satisfies it; a harness that would *let a future cycle* investigate does not. Full reasoning and its scope (a routine-level interpretation, overturnable by the operator) in `cycle-159-verdict.md`.
