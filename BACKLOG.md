...
- [ ] BACKLOG-033 [art] Grass / path / water tiles — **Gen3 pixel** (CHARTER v4), 16×16 grids baked ×2, seamless *(progress: 1/3 — **grass** shipped cycle 048-art, 2026-06-13: `art/tileArt.ts` `GRASS_RIG` (a pure `TileRig` format — fixed ground palette, spatial variants not anim frames) + `bake.ts` `bakeTileMap` (bakes the whole COLS×ROWS floor to ONE static texture, variants alternating like the old checker — zero per-frame cost vs the prior live Graphics) + `drawGrassMap` swapped to it (flat checker kept as graceful fallback). Flat field green scattered with darker blade tufts (light tip over a two-pixel stem), two variants; every tile border is flat base so it tiles seamlessly in any A/B arrangement (unit-pinned). 7 unit + 2 e2e; 431 unit / 162 e2e green, no flake. **Remaining: path + water** — deferred to the zone that needs them (BACKLOG-143); they have nowhere to render on the all-grass bowl yet.)*
