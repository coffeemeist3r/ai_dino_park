...
- [ ] BACKLOG-474 [core] The unsettled ground — a zone added to the chain today would arrive pre-populated or not at all; give the park a way for a ground to *fill up*. A zone may open with **zero residents and no provider**, and gain its population only through the migration spine (450/460): the first dino to settle there founds it, and the first to bank a harvest into its store becomes its first provider (448). The work is making every per-zone read sane at population 0 — prosperity (428) at an empty ground, the decline floor (460) which currently guarantees a zone never empties, the pantry (446) with no mouths, the lens box (425/468) with no names — plus a migration pull that will actually *aim* at an empty ground instead of the richest one. The proof M10 wants: a ground the player watches get its first resident. Builds on 472 / 450 / 460 / 448.
- [ ] BACKLOG-475 [core] Distance on the chain — adjacency (084/449) is a flat neighbour list, which was indistinguishable from "the whole park" while the chain was three long. At four, two grounds can sit three hops apart, and every cross-zone system silently assumes one hop: the ferry (447) picks *a* lighter neighbour, the demand read (438) points at *a* grower, migration (450) walks to *the richest*. Give the chain a derived hop-distance table off the adjacency graph and let those three reads prefer the *nearest* qualifying ground, so the far end of the chain is genuinely far and food doesn't teleport across the park. Builds on 449 / 447 / 438 / 450.
