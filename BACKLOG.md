...
- [ ] BACKLOG-531 [art] The mark of somebody who noticed — the fourth glyph in 520's hour-and-mood family, and the first one that is about the *keeper* rather than the hour. `doze` is an eye shut, `rouse` is that eye open, `vigil` (526) is a pair of eyes turned out of the screen. This is the beat after being looked at: a small thought-mark a dino wears for a few seconds on the keeper's return, when the absence left it something to say. One glyph, graded in **two** steps rather than three, because the third grade in this family is *nothing at all* — the dino that never noticed you were gone wears no mark, and that absence is the read. Family constraints are the ones 526 established and should be asserted, not described: `rouse`'s outline and ground values exactly, a lit-cell count below `vigil`'s 76 (this is a smaller thought than a watch), and a catchlight dimmer than `rouse`'s, which stays the brightest pixel in the park's hour-marks. Host ships the same cycle (BACKLOG-116), so this is drawn, not stashed.
