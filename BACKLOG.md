...
- [x] BACKLOG-515 [infra] The suite's serial/parallel split — two unrelated specs now fail at `--workers=1` and pass under load (full text in the cycle-144 block below).
...
- [x] BACKLOG-524 [emergent] The night shift — 109 gave the park a cast that is up while it is dark, and no system in the park knows they are there (full text in the cycle-147 block below).
...
- [x] BACKLOG-525 [art] The three that sleep undrawn — BACKLOG-522 gave `PixelRig` an optional `down` pair and drew two of five species; **brontosaurus, compsognathus and parasaurolophus still stop mid-stride when they rest.** Draw the remaining three down poses to the constraints 522's own unit test already pins as numbers: topmost painted row strictly *below* the standing frame's, bottom row *exactly equal* to it (a sleeper whose feet leave its tile is the bug nothing else in this park can see), at least three quarters of the standing frame's painted mass retained (it must read as the same animal lying down, not as a different dino), two frames baked at 1.5fps against the walk's six. Each keeps its species' silhouette feature upright and settles everything under it — the plate ridge and the frill were 522's two solved cases and are the pattern to copy. Host shipped last night and is not in question: `isResting` (109) puts sleepers down on frame one, `setAsleep` swaps the texture on the state edge, and `down` stays optional so the standing-frame fallback survives for anything undrawn. Reachable on a fresh save: Pip (grove), Thornback (fernreach) and Ember (ridge) are all owls, all asleep at 08:00, and two of the three are the only resident of their ground. **Note for the Artist:** with all five drawn, the rectangle fallback control no longer rides on an undrawn species here — it rides on `down` being optional, which the unit test must keep asserting through a deliberately posed-and-unposed pair.
...
- [x] BACKLOG-110 [social] Hour-aware greeting — a dino's first player line of the real day leans on the hour (a yawn near dawn, a sleepy note at night) layered onto the existing context prompt. Small living touch that realtime makes meaningful.
...
- [x] BACKLOG-279 [emergent] Greeting carries the hour — a fond dino's named hello leans on the time of day ("up early too, AETHER-1?"), layering 110's hour-aware idea onto the 272/276 greeting. Builds on 272 / 276 / 051.
...
> Two findings from the build rather than the plan. 515 is the runner property BACKLOG-430 has been
...
- [x] BACKLOG-515 [infra] The suite's serial/parallel split — `mobile-minds` has failed at `--workers=1` on a stashed clean HEAD since cycle 135, and cycle 144 found **`cycle-044-sound`** ("a greeted dino answers in its own voice") doing exactly the same thing: it failed in one full parallel run, passed in the next, and fails at `--workers=1` on a stashed clean HEAD with that cycle's entire diff removed. Two unrelated specs, on opposite sides of the codebase, both **failing serial and passing under load** — which is the inverse of the parallel-load theory the studio carried from cycle 92 to cycle 135, and which makes this a property of the runner rather than of either spec. BACKLOG-430 is now one symptom of this item, not the whole of it, and should be re-pointed at it rather than re-diagnosed a fourth time. Both known failures are at a *timing* seam a fast serial boot reaches sooner than a loaded one — 430 at `__keeperPickerOpen()` (the picker is not open where the spec expects), 044 at `__lastSound()` returning null after a tone pick whose reply resolves async. The likely shape of the fix is a shared "wait for the thing that must have happened" helper beside `boot()` in `tests/e2e/helpers.ts`, rather than a `waitForTimeout` per spec — the same argument BACKLOG-495 makes about founding state, applied to time. Reproduction for either: `git stash -u && npx playwright test <spec> --workers=1`. The cost of leaving it: the full run reads 619/620 rather than all-green, so a genuine regression can hide behind a standing red — which is the reason 430 was filed in the first place, three items ago. Builds on 430 / 486 / 495. **(2026-08-30, cycle 145: a third spec, failing in the *other* direction. Both `controls-help` specs failed at `boot()`'s `__ready` wait in a full parallel run and passed 3/3 in an isolated re-run seconds later — pass serial, fail under load, the inverse of the two above. Folded in here rather than filed as a fourth item, because a runner that loses specs in both directions is one problem and not three. The shared-helper fix this item proposes now has to cover both: something `boot()` waits for is not reliably true at either extreme of load.)**

## Cycle 147 structure additions (2026-09-01)

- [ ] BACKLOG-523 [core] The hour a save opens on — the founding clock starts at **08:00** and that number is a literal, written once, derived from nothing and pinned by nothing. It is now the single most load-bearing founding constant in the park and nobody has noticed: BACKLOG-109's whole reachability answer is *"Rex is asleep at eight in the morning"*, which is a claim about the opening hour and not about chronotypes; BACKLOG-493's session claim is measured in in-game minutes from it; 520's two hour-marks, the huddle window, the crops, spoilage, upkeep and the council's term all read a clock whose zero is this. Move it by four hours in either direction and the park's frame-one read changes completely — in one direction every dino is up and the milestone's arcs go dark, in the other the park opens in the dark. Nothing would fail. The work: derive the opening hour from what the park claims to show at it rather than asserting it, give it a **reachability register entry** of the shape 501 established (*"the save opens inside the window where the cast is split"* — routed through `atRest`/`chronotypeOf` for the shipping roster, never restating 08:00), and put the founding hour and `ACTIVE_SCALE` beside each other so the two halves of "what fits in a session" are read together. This is 495's argument and 519's class applied to *time's origin* rather than to founding state or to a copied number — the seam cycle 146 arrived at twice from different directions and did not name. Builds on 493 / 501 / 109 / 519.
- [x] BACKLOG-524 [emergent] The night shift — BACKLOG-109 split the cast into day-dinos and night-owls, and **no system in the park knows the owls exist.** Every system that does work does it on a world step with no idea who is up: gathering, carrying, mending, building, the ballot, the migration tiers, the hatch. So an owl awake at midnight does exactly what it would do at noon, and a park whose whole daytime workforce is asleep still gathers at full rate, because the systems count residents and not *waking* residents. That is the same defect CHARTER v7 was written about, one layer along: a distinction that is true, tested and load-bearing, and that changes nothing you can watch. The work is one seam, not a rewrite — a single `awakeIn(zone)` read that the ground's work systems take instead of a raw resident count, so a ground's output shape follows its cast's hours; plus one thing an owl does *because* it is the only one up, so the night is a shift and not just a tint over the same behaviour. The reachability read is the one the milestone asks for: stay past nightfall and the park is doing less, and the dino still moving is doing something the day-dinos do not. Blocked on nothing; 109 shipped the split. Builds on 109 / 520 / 486.
