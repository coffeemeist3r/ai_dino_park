...
- [~] BACKLOG-456 [infra] Pinned piles vs. ambient gathering — **promoted into the Structure Track cycle 125** on the cycle-124 verdict's standing recommendation (three consecutive cycles of surfacing; two full suite runs spent last cycle). Load-bearing infra by the routine's own test — a system every future e2e builds on — and Milestone 12 opens by adding a fifth ground to the same suite that is already flaking under parallel load. Full item text in `## Infra` below.
