...
- [~] BACKLOG-463 [emergent] The provider's say — the resources→crafting→building→**governance** arc (CHARTER) has stalled at *building*: a zone can raise a granary (454) and crown a provider (448), but no dino ever *decides* anything for the ground. Seed the first governance beat, foundation-first: a zone with a standing provider gains one persistent, provider-set **spend priority** (feed-the-hungry-first vs. bank-for-a-granary) that the pantry-spend (444) and the auto-build gate read, so who a zone trusts to keep it fed also shapes *how* it spends. One pure per-zone policy value on the save, set by the provider on the role cadence, read by two existing hooks — not a voting system (that stays 031). Builds on 448 / 454 / 444 / 031.
- [ ] BACKLOG-467 [emergent] The say changes hands — when a zone's provider role passes to a new dino (one out-banks the incumbent, 448), the incoming provider re-sets the spend priority (463) from *its own* temperament and the handover lands a one-off logged governance beat ("Sunny sets the Grove's table now"), so *who holds the say* — and the moment it changes — is legible, not silent. Foundation follow-up to 463: no new policy, just making the policy-setter's turnover a visible beat. Builds on 463 / 448 / 453.
- [ ] BACKLOG-468 [emergent] The provider's read on the lens — the per-zone spend priority 463 sets lives only in two hooks; surface it on the zone-map lens beside the prosperity tier (🍽️ feeds-first / 🏦 banks-toward-plenty) so the player can *see* how each ground has chosen to spend, the governance beat made as readable as the pantry line already is. A pure lens read off the persisted policy — no new state. Builds on 463 / 428 / 433.
