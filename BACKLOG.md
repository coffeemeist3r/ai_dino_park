...
- [ ] BACKLOG-554 [art] The fourth watcher, in pixels — the keeper roster has rendered three drawn observers and one amber square since cycle 047-art, and the square is the deliberate rectangle-fallback control (`__hasKeeperArt('vex-0') === false`, the pterodactyl convention). **BACKLOG-212 adds a fourth *real* roster entry — the first watcher that is not a time-traveling robot — and it ships on that same amber square.** Draw it: a 16x20 `keeperArt.ts` rig in the format Aki/Vix/Lux already use, baked through `makeKeeperArt`/`ensureKeeperWalk` and blitted by `renderKeeperAvatar()`, which rebuilds on a `K`-pick or a save restore. **The host is live and predates this seed by 116 cycles** — this is not a 543/539-shaped host-blocked item, it is the 158 pipeline being handed a fourth id. Two conditions. (1) It ships only once 212's roster entry exists, per the cycle-145 amendment: a rig with no id to key off is an unplaced rig and a red build. (2) The fallback control must be re-pointed *before* the bake, exactly as cycles 045/046/047 did each time the roster's undrawn survivor changed — `vex-0` is already a genuine no-art id and should stay the control, so this one is cheap, but check it rather than assume it. **Draw against the robots, not with them.** Aki is a calm two-row visor, Vix a hostile one-row slit, Lux a round archive lens — all three read as *optics on a chassis*. The fourth is the roster's first organic-category watcher and its silhouette is the whole point of 212: it must be identifiable at 16x20 as *not a machine*, from the head shape alone, before any palette lands. Same GBA discipline as the rest of the family (fixed per-observer palette, dark-but-never-black outline, stand/step-L/step-R at `[0,1,0,2]`). Builds on 212 / 158 / 155 / 168.
