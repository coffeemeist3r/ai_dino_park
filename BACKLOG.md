...
- [ ] BACKLOG-431 [infra] Pause ambient timers in e2e — **operator-injected 2026-07-06, ship next cycle ahead of 418**: CI e2e reds are wall-clock ambient timers (migration ~90s, edge-barter, sky, chorus) racing slow parallel CI runners; add a `__pauseAmbient()` dev hook that freezes them, wired into e2e `boot()` in `tests/e2e/helpers.ts`. Full text under Infra below.
...
- [ ] BACKLOG-431 [infra] Pause ambient timers in e2e — **operator-injected 2026-07-06.** CI keeps going red on a *different* spec each run (`cycle-077-carry` on ffbfee3, `cycle-028-realtime` locally) while every one passes 3/3 in isolation — a flake class, not regressions. Root cause: 300+ Playwright specs run full-parallel against one dev server, and the sim's wall-clock background timers (ambient migration ~90s, edge-barter, sky events, dawn chorus) keep mutating world state mid-test; slow CI runners stretch the gap between a spec's setup and its assert, the world ticks past the pinned state, `Expected 1 Received 0`. Retries lose both attempts because the slow environment persists. Fix at the root: a `__pauseAmbient()` dev/test hook (exposed like the existing e2e hooks) that freezes the real-time ambient timers, called from the shared e2e `boot()` in `tests/e2e/helpers.ts` so all specs get it for free — specs that *test* an ambient beat opt out or step time explicitly. Kills the whole race class in one place; `cycle-077-carry`'s per-spec intent-pinning band-aid becomes unnecessary (may stay). Player-facing behavior unchanged — hook is test-only, never wired into normal play.
