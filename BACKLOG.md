...
- [x] BACKLOG-114 [pokemon] Away-log in the book — the collection book keeps the last "while you were away" digest so you can re-read what the bowl got up to. Builds on 106 + 021. **(Shipped cycle 153 — three returns, not one, newest first, above the cast; persisted, so re-read means tomorrow.)**
...
- [x] BACKLOG-535 [core] The stake's undecided driver — BACKLOG-518 has been held out of the Artist's hands for seven consecutive fires for one reason, and it is not an art reason: the fourth founder's-stake state is *"the ground is still being looked after"* and **no number in this park means that**. 513 reads who founded a ground and 514 reads that everybody left, both off `foundingPioneers`/resident count; the third axis has no source, so the rig has no host, so the cycle-145 amendment keeps it queued. Decide it here, in the structure lane where it belongs, rather than in an art fire that will pick whatever is nearest. The honest candidate the last three chronicles keep naming is `pileStep` off the ground's own bank — 504/506 already prove three steps read at 32px, and after 495 three grounds boot with different banks, so a bank-keyed stake would say three different things on a first frame instead of one. The competing candidates are the upkeep ledger (a ground with no derelict landmark is a ground being kept up — arguably the truer reading of *looked after*, and after 528 it is a number that finally moves) and the prosperity index (428). Pick one, write down why the other two lost, expose it as a pure `stakeUpkeepStep(zone)` beside `stake.ts`'s existing reads, and wire the host. The art follows on the next fire. Builds on 513 / 514 / 518 / 504. **(Shipped cycle 153 — the winner is the upkeep ledger, `standing > 0 && derelict === 0`. `pileStep` lost because 504 already draws that number as a heap on the same ground; prosperity lost for being a composite. Precedence hollowed > kept > kind. 518 is drawable.)**
