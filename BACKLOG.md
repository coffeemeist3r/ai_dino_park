...
- [ ] BACKLOG-548 [art] The last two plots read as emoji — `ripeRigKey` (`plot.ts:70`) maps a zone's crop to `crop_ripe_<food>`, `drawPlotSprite` bakes that rig where it exists and falls back to the glyph where it does not, and three of the five grounds get pixels: the Bowl's berry (`crop_ripe`, 317), the Grove's greens (418) and the Fernreach's roots (432). The Hollow's pale mushrooms (472) and the Ridge's hard pine seeds (478) have never been drawn, so a keeper who walks east past the Fernreach watches the farming arc *downgrade* — two grounds whose ripe plot is a 🍄 and a 🌰 sitting in a pixel world beside a pixel sprout. **The host already exists and has since 434.** This is not a stash-ahead rig and it is not blocked on anything: add `CROP_RIPE_MUSHROOMS_RIG` and `CROP_RIPE_SEEDS_RIG` to `PROP_RIGS` under the keys `worldPlacedProps()` already registers for them (`reachability.ts:136` walks `zoneChain()` and asks for exactly these two), and both bake on the next boot with no wiring at all. Draw them the way 432 drew the roots — distinct in silhouette from each other, from the shared sprout, and from the three ripe crops already on the board, and legible at plot scale against the Hollow's damp fen and the Ridge's bare high ground rather than against the Bowl's grass. Completes the per-zone crop set; the first `[art]` item in four fires whose host is already in the code. Builds on 434 / 432 / 472 / 478.
