...
- [ ] BACKLOG-545 [core] Once per sitting — the session record as a gate, so a beat that should happen once a visit does (full text in the cycle-156 block below).
- [ ] BACKLOG-533 [infra] The fixture nobody is required to name — 495 built the seam and moved the whole suite onto it; nothing yet makes a spec *declare* its founding state. **Passed over three times (155, 156, 157); ranked below 538 at cycle 157 because an item waiting on evidence nobody is generating should not outrank an item generating evidence every cycle. Its cycle-158 deadline stands, unchanged and now due: next cycle's Structure-smith must either rewrite the entry condition or take the item** (full text in the cycle-151 block below).
