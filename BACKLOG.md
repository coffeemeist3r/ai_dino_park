...
- [ ] BACKLOG-105 [core] Wall-clock-anchored time + configurable scale — replace the tick-counting `WorldClock` ([clock.ts](game/src/world/clock.ts)) with time derived from `Date.now()` each frame (anchor epoch + scale multiplier persisted in the save), so in-game time is always correct even when the Phaser timer throttles in a background tab. Default scale = **1× realtime** (full day/night = 24 real hours, true fishbowl); expose a scale knob (e.g. 1× / 60×) for active watching. `tick`/`onHour`/`onTick` semantics preserved — derive elapsed in-game minutes from the wall-clock delta and fire boundaries that were crossed. Cornerstone: unblocks BACKLOG-106. Keep it pure/Node-testable (inject a `now()` clock source).
