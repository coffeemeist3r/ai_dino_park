...
- [~] BACKLOG-523 [core] The hour a save opens on — 08:00 is a literal nobody derives, and every day-shaped read the park now ships is measured from it (full text in the cycle-147 block below). **(in flight, cycle 149)**
...
- [ ] BACKLOG-529 [core] The keeper's own clock is not the park's — the vigil is the first system to read the wall clock's hour as a thing the park reasons about, and it does it through a bare `new Date()` (full text in the cycle-149 block below).
...
- [ ] BACKLOG-526 [art] The one who waited — the vigil mark for BACKLOG-121. When you open the park at the hour you usually open it, a waking dino walks to the hatch and stands facing the glass; the beat ships tonight wearing 👀, the way 109's sleeper and owl shipped wearing 💤 and 👁 before 520 drew them. Draw the pose, not a prop: a dino turned out of the screen, head up, weight forward, so the read is **attention aimed at the player** — the one thing no frame in this park's whole sprite set currently carries. Two frames at the resting cadence (1.5fps), the silhouette-feature rule 522/525 established (the species' identifying feature stays up), and the mass floor from 525 with its lesson applied: for a pose that is *standing still*, the honest target is about the same mass as the walk frame, not merely three quarters of it. Slot family is 520's — mutually exclusive with 💤 by construction, and it **takes precedence over 👁** when a dino is both an owl at night and the one keeping the vigil, because what it is doing beats what hours it keeps. Host shipped and reachability-pinned the same cycle this was filed. Builds on 121 / 520 / 525.
