...
- [ ] BACKLOG-560 [art] The watch line's register — the tenure mark. `Watch · ` (555) is the first of the three keeper lines and the only one of them still bare: 539 engraved the day-count, and the plaque's per-line `Text` objects (558) are the host, so this one is drawable tonight rather than blocked. The subject is a **watch-glass** — a ring with a single hand struck through it — because the line counts a span of tenure, not an event, and because no rig in the registry owns a circle with a radius in it. Same register as 539: struck into brass, lit lip `#fff1c9`, face `#f4d58d`, cut darker than the panel, no `rouse` outline (this family is engraving, not marking). Finds its row by the `Watch · ` prefix, exported from `ui/plaque.ts` beside `STREAK_PREFIX` — an index would engrave the wrong line the first time an optional line above it went away. `worldPlacedProps` gains its second **chrome** entry, for 539's reason. Builds on 555 / 539 / 558.
- [ ] BACKLOG-561 [art] The sitting line's register — the hourglass. `Sitting · ` (542) is the middle keeper line and reads about *now* where 560's reads about the whole tenure, so the two want marks that differ at 16px: a ring for the span, a waist for the session. The subject is an **hourglass** — two stacked triangles meeting at a two-cell neck, grains below. The floor 539 learned the hard way applies and the spec asserts it: nothing in the rig is thinner than two cells against 11px type. Lit-cell count claimed against every sibling, as every rig since 146 has been. Same brass register, same prefix-not-index rule, same chrome entry. Takeable in the same fire as 560 or the one after. Builds on 542 / 560 / 539 / 558.
