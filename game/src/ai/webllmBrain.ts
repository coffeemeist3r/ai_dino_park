...
export function buildMessages(ctx: NPCContext, obs: Observation): { role: string; content: string }[] {
  ...
  const system =
    `You are ${ctx.name}, a ${ctx.species} dinosaur living in a peaceful park. ` +
    `Your personality is: ${persona}. ` +
    `Always stay in character. Reply with a single short, natural sentence — no narration, no quotes.`;
...
export class WebLLMBrain implements NPCBrain {
  ...
  private async generate(engine: ChatEngine, ctx: NPCContext, obs: Observation): Promise<Reply> {
    ...
    if (!raw) return cannedReply(ctx);
    return { text: raw.slice(0, MAX_REPLY), mood: moodFromTraits(ctx.traits) };
  }
}
