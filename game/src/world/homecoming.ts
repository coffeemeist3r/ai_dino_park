...
/** A runner-up this close (in points) to the closest dino feels overlooked — one heart's worth (10 pts/heart). */
export const JEALOUS_TIE_POINTS = 10;

/** The runner-up's flash of jealousy when a near-tied rival gets the welcome-back (BACKLOG-120). */
export interface Jealousy {
  /** the second-closest dino — the one who feels slighted. */
  name: string;
  /** the floating sulk line (contains the name + 😒). */
  line: string;
  /** the faint memory the slighted dino keeps; WorldScene folds this into the memory store. */
  memory: string;
}
...
export interface Homecoming {
  ...
  memory: string;
  /** a near-tied runner-up's sulk, or null when no one's close enough to be jealous. */
  jealous: Jealousy | null;
}
...
export function homecoming(friendship: Friendship, awayMinutes: number): Homecoming | null {
  ...
  const runnerUp = topBy(friendship, best.name);
  const jealous: Jealousy | null =
    runnerUp && best.points - runnerUp.points <= JEALOUS_TIE_POINTS
      ? {
          name: runnerUp.name,
          line: jealousLine(runnerUp.name),
          memory: `the keeper fussed over ${best.name}`,
        }
      : null;
