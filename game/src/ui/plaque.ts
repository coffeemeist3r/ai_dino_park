...
export interface PlaqueStats {
  ...
  upkeep?: string;
  /** Who is watching, and for how long (BACKLOG-555), from `watchLine`. Absent/empty → no line.
   *  First of the three keeper lines: this one is about the whole tenure, the two below it are about
   *  this visit, so they read outward from the longest span to the shortest. */
  watch?: string;
 ...
 * ground owes a day (536), who is watching and since when (555), how long this sitting has run (542)
 * and how many days running the keeper has turned up (122).
 *
