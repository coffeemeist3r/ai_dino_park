...
test('the full roster sweep: every observer is sprite-backed, no survivor on the square', async ({ page }) => {
  ...
  // Narrowed to the three robots for the length of cycle 163's morning, when BACKLOG-212 added a seat that
  // shipped undrawn. **Restored the same night by BACKLOG-554, as its note promised** — whole-roster
  // equality is the assertion that stops a watcher shipping as a rectangle unnoticed.
  expect(arts).toEqual({
    aether: 'keeper_aether_walk',
    vanta: 'keeper_vanta_walk',
    lumen: 'keeper_lumen_walk',
    kestrel: 'keeper_kestrel_walk',
  });
});
